...
1. 這份檔案是唯一來源。改這裡就好,不要另外維護第二份。
2. 每做完一件使用者「感覺得到」的事,就寫進最上面的〈未發佈〉區塊。
   使用者感覺不到的(重構、抽模組、改檔名)不要寫,那些留給 handoff。
3. 發版時把〈未發佈〉的標題改成 `## 版號 · YYYY-MM-DD`,再跑 release.py。
   release.py 會自動拿「最上面那個版號區塊」當 GitHub Release 的說明,
   也會把這份檔案和 images/ 同步到公開的 releases repo 當圖文網頁。
4. 標題格式必須是 `## 6.2.1 · 2026-09-02`(版號 + 空格點空格 + 日期),
   程式靠這行切版本。非版號的 `##` 標題(像〈未發佈〉)會被自動略過。

放圖 / GIF(照片、截圖、操作動畫都可以):
    ![左邊是舊的,右邊是新的](images/6.3.0-changelog-window.png)
    ![按一下就自動更新完](images/6.3.0-autoupdate.gif)
  - 圖檔放 docs/images/,檔名用「版號-英數說明」—— 中文檔名在網址裡要跳脫,
    容易出事,說明文字寫在中括號裡就好(那段中文會顯示在圖片下方)。
  - 路徑一定要寫相對的 `images/xxx`,這樣在網頁上才找得到。
  - 程式內的視窗是純文字版,圖會自動被拿掉,不會出現破圖。
  - 單張建議 2 MB 以內、寬度 1200px 左右;GIF 盡量 10 秒內。
  - 放進 docs/images/ 就好,發版時 release.py 會自動推上去,不必自己上傳。
-->
...
- 多了「說明 → 更新紀錄」,可以直接在程式裡看每一版改了什麼,不用再去問人。
- 更新紀錄視窗下方有「看圖文版」,按下去會用瀏覽器打開同一份紀錄的網頁版,
  那邊會附上截圖和操作動畫。
- 發現新版時跳出來的提示,現在會直接寫出這一版改了什麼,不再只有版號。
